...
if [ "$(uname)" != "Darwin" ]; then
  mjpython phc_mjx/run.py env="env_im" exp_name=im_obsv2_standing \
    env.motion_file=sample_data/amass_isaac_standing_upright_slim.pkl \
    headless=False test=True epoch=-1
else
  python phc_mjx/run.py env="env_im" exp_name=im_obsv2_standing \
    env.motion_file=sample_data/amass_isaac_standing_upright_slim.pkl \
    headless=False test=True epoch=-1
fi
