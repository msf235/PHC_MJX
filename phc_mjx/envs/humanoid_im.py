...
class HumanoidIm(HumanoidTask):
    def __init__(self, cfg):
        self.num_traj_samples = cfg.env.get(
            "num_traj_samples", 1
        )  # paramter for number of future time steps
    ...
    def draw_task(self):
        def draw_obj(scene):
            ...
            ref_dict = self.get_state_from_motionlib_cache(
                self._sampled_motion_ids, time, self.global_offset
            )
    ...
    def setup_motion_data(self):  # should this be named setup_motion_sampler?
        if self.motion_file_type == "smpl":
            ...
            self.motion_file_type == "mujoco"
        ):  # Calling this 'mujoco' for consistency, but probably should be called 'spatial' or something
            self.setup_motion_mujoco()
        else:
            raise NotImplementedError("Unknown motion file type")
    ...
    def setup_motionlib(self):
        ...
        if self.test:
            self.motion_lib.load_motions(
                self.motion_lib_cfg, shape_params=self.gender_betas, random_sample=False
            )
        else:
            self.motion_lib.load_motions(
                self.motion_lib_cfg,
    ...
    def forward_motions(self, num_threads=32):
        for motion_start_idx in range(
            0, self.motion_lib.num_all_motions(), num_threads
        ):
            self.motion_start_idx = motion_start_idx
            self.motion_lib.load_motions(
                self.motion_lib_cfg,
                shape_params=self.gender_betas * num_threads,
                random_sample=False,
                silent=False,
    ...
    def next_motions(self):
        ...
        self.motion_lib.load_motions(
            self.motion_lib_cfg,
            shape_params=self.gender_betas * self.cfg.num_threads,
            random_sample=False,
            silent=False,
            start_idx=self.motion_start_idx,
        )
    ...
    def get_task_obs_size(self):
        if self.im_obs_v == 1:
            obs_size = (
                len(self.track_bodies) * self.num_traj_samples * (3 + 6 + 3 + 6)
            )  # 3 for position, 6 for rotation, 3 for ref position, 6 for angular ref rotation
            obs_size += (
                1 + 1 + len(self.track_bodies) - 1
            ) * 3  # root velocity, root angular velocity, dof velocity
        elif self.im_obs_v == 2:
            assert self.cfg.robot.create_vel_sensors
            obs_size = (
                len(self.track_bodies) * self.num_traj_samples * (3 + 6 + 3 + 3 + 3 + 6)
            )  # 3 for position, 6 for rotation, 3 for linear vel, 3 for angular vel, 3 for ref position, 6 for ref rotation
    ...
    def compute_reset(self):
        ...
        terminated = compute_humanoid_im_reset(
            ...
            use_mean=self.im_eval,
        )[0]
        truncated = (
            time > self.motion_lib.get_motion_length(self._sampled_motion_ids)
        )[0]
    ...
    def reset_task(self, options=None):
        if self.test:
            ...
            self._motion_start_times[:] = self.motion_lib.sample_time(
    ...
    def compute_task_obs(self):
        ...
        if self.im_obs_v == 1:
            ...
            task_obs = compute_imitation_observations_v2(
                root_pos,
                root_rot,
                body_pos_subset,
                body_rot_subset,
    ...
    def recrod_eval_states(self):
        ...
        motion_res = self.motion_lib.get_motion_state_intervaled(
            self._sampled_motion_ids, motion_times.copy(), offset=self.global_offset
        )
    ...
    def dump_record_eval_states(self):
        return_dict = {
            self.motion_lib.curr_motion_keys[self._sampled_motion_ids][0]: {
                k: np.array(v) for k, v in self.state_record.items()
            }
        }
    ...
    def compute_reward(self, actions):
        ...
        if self.im_reward_v == 1:
            ...
            reward, reward_raw = compute_imitation_reward_v2(
                qpos,
                qvel,
                body_pos_subset,
                body_rot_subset,
    ...
    def compute_power_reward(self):
        return (
            -self.power_coefficient * np.array(self.curr_power_usage).mean(axis=0).sum()
        )
...
def compute_imitation_observations_v1(
    qpos,
    qvel,
    body_pos,
    body_rot,
    ...
    heading_rot_expand = np.tile(
        heading_rot[..., None, :, :].repeat(body_pos.shape[1], axis=1),
        (time_steps, 1, 1),
    )
    ...
    diff_global_body_rot = npt_utils.quat_mul(
        ref_body_rot.reshape(B, time_steps, J, 4),
        np.tile(npt_utils.quat_conjugate(body_rot), (time_steps, 1, 1)).reshape(
            B, time_steps, J, 4
        ),
    )
    ...
    diff_local_body_rot_flat = npt_utils.quat_mul(
        npt_utils.quat_mul(
            heading_inv_rot_expand.reshape(-1, 4), diff_global_body_rot.reshape(-1, 4)
        ),
        heading_rot_expand.reshape(-1, 4),
    )  # Need to be change of basis

    obs["diff_local_body_pos"] = diff_local_body_pos_flat  # 1 * J * 3
    obs["diff_local_body_rot"] = npt_utils.quat_to_tan_norm(
    ...
    diff_root_vel = root_velp.reshape(B, time_steps, 1, 3) - ref_velp.reshape(
        B, 1, 1, 3
    )
    diff_root_ang_vel = root_velr.reshape(B, time_steps, 1, 3) - ref_velr.reshape(
        B, 1, 1, 3
    )

    obs["diff_local_root_vel"] = npt_utils.quat_rotate(
        heading_inv_rot.reshape(-1, 4), diff_root_vel.reshape(-1, 3)
    )
    ...
    local_ref_body_rot = npt_utils.quat_mul(
        heading_inv_rot_expand.reshape(-1, 4), ref_body_rot.reshape(-1, 4)
    )
    obs["local_ref_body_rot"] = npt_utils.quat_to_tan_norm(
        local_ref_body_rot
    )  #  1 * J * 6
...
def compute_imitation_observations_v2(
    ...
    diff_global_ang_vel = ref_body_ang_vel.reshape(
        B, time_steps, J, 3
    ) - body_ang_vel.reshape(B, 1, J, 3)
    obs["diff_local_ang_vel"] = npt_utils.quat_rotate(
        heading_inv_rot_expand.reshape(-1, 4), diff_global_ang_vel.reshape(-1, 3)
    )
...
def compute_imitation_reward_v1(
    qpos, qvel, body_pos, body_rot, ref_body_pos, ref_body_rot, ref_qvel, rwd_specs
):
    ...
    diff_global_body_rot = npt_utils.quat_mul(
        ref_body_rot, npt_utils.quat_conjugate(body_rot)
    )
    ...
    reward_raw = {
        ...
        "r_body_rot": r_body_rot,
        "r_root_vel": r_root_vel,
        "r_root_ang_vel": r_root_ang_vel,
        "r_dof_vel": r_dof_vel,
    }
...
def compute_imitation_reward_v2(
    qpos,
    qvel,
    body_pos,
    body_rot,
