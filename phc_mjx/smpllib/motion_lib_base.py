...
from phc_mjx.utils import torch_utils
import joblib
import torch
import torch.multiprocessing as mp
import copy
import gc
from phc_mjx.smpllib.smpl_parser import (
    SMPL_Parser,
    SMPLH_Parser,
    SMPLX_Parser,
)
...
class MotionLibBase:
    def __init__(self, motion_lib_cfg):
        ...
        self.setup_constants(
            fix_height=motion_lib_cfg.fix_height,
            multi_thread=motion_lib_cfg.multi_thread,
        )
    ...
    def load_motion_with_skeleton(
        ids, motion_data_list, shape_params, mesh_parsers, config, queue, pid
    ):
    ...
    def load_motions(
        self, m_cfg, shape_params, random_sample=True, start_idx=0, silent=False
    ):
        ...
        if random_sample:
            sample_idxes = np.random.choice(
                ...
                p=self._sampling_prob,
                replace=True,
            )
        else:
            sample_idxes = np.remainder(
                np.arange(num_motion_to_load) + start_idx, self._num_unique_motions
            )
        ...
        ).astype(self.dtype)  # xpos
        self.grs = np.concatenate([m.global_rotation for m in motions], axis=0).astype(
            self.dtype
        )  # xquat
        ...
        if not silent:
            print(
                ...
                f"total length of {total_len:.3f}s and {self._motion_num_frames} frames.",
            )
        else:
    ...
    def sample_motions(self, n=1):
        motion_ids = np.random.choice(
            np.arange(len(self._sampling_batch_prob)),
            size=n,
    ...
    def get_motion_num_steps(self, motion_ids=None):
        if motion_ids is None:
            ...
            return (self._motion_num_frames[motion_ids] * 30 / self._motion_fps).astype(
                int
            )
    ...
    def get_motion_state_intervaled(self, motion_ids, motion_times, offset=None):
        ...
        fl = self._calc_frame_interval(
            motion_ids, motion_times, motion_len, num_frames, dt
        )
