...
from phc_mjx.utils.transformation import euler_matrix
...
class Skeleton:
    ...
    def construct_tree(
        self,
        template_fname=files("phc_mjx").joinpath(
            "data/assets/mjcf/humanoid_template_local.xml"
        ),
    ...
    def add_vel_sensors(self, bone, sensor_node, sensor_type="framelinvel"):
        SubElement(
            ...
            sensor_type,
            {
                "name": "sensor_" + bone.name + f"_{sensor_type}",
                "objtype": "xbody",
                "objname": bone.name,
            },
        )
        ...
        if bone.parent is None:
            ...
            if self.ball_joints:
                ...
                for i in range(len(bone.dof_index)):
                    ...
                    j_attr["type"] = "hinge"
                    j_attr["pos"] = "{0:.4f} {1:.4f} {2:.4f}".format(
                        *(bone.pos + offset)
                    )
        ...
        if g_attr["type"] == "capsule":
            ...
            if bone.name == "L_Toe" or bone.name == "R_Toe":
                if self.upright_start:
                    pos[2] = (
                        -bone.pos[2] / 2
                        - self.size_buffer[bone.parent.name][2]
                        + size[2]
                    )  # To get toe to be at the same height as the parent
                    pos[1] = (
                        -bone.pos[1] / 2
                    )  # To get toe to be at the same x as the parent
                else:
                    pos[1] = (
                        -bone.pos[1] / 2
                        - self.size_buffer[bone.parent.name][1]
                        + size[1]
                    )  # To get toe to be at the same height as the parent
                    pos[0] = (
                        -bone.pos[0] / 2
            ...
            if not self.remove_toe:
                ...
                rot = sRot.from_euler(
                    "xyz", [0, 0, np.arctan(bone_dir[1] / bone_dir[0])]
                ).as_quat()[[3, 0, 1, 2]]
            ...
            if self.big_ankle:
                ...
                if bone.name == "L_Toe" or bone.name == "R_Toe":
                    parent_min, parent_max = (
                        self.hull_dict[bone.parent.name]["norm_verts"]
                        .min(axis=0)
                        .values,
                        self.hull_dict[bone.parent.name]["norm_verts"]
                        .max(axis=0)
            ...
            if self.real_weight_porpotion_boxes:
                g_attr["density"] = str(
                    (hull_params["volume"] / (size[0] * size[1] * size[2] * 8).item())
                    * base_density
                )
