...
class MotionLibMujoco(MotionLibBase):
    ...
    def fix_trans_height(
        ...
        with torch.no_grad():
            ...
            if fix_height_mode == FixHeightMode.ankle_fix:
                ...
                diff_fix = (
                    vertices_curr[:, pick][:frame_check, ..., -1].min(dim=-1).values
                    - height_tolorance
                ...
                diff_fix = (
                    vertices_curr[:frame_check, ..., -1].min(dim=-1).values
                    - height_tolorance
                ).min()  # Only acount the first 30 frames, which usually is a calibration phase.
    ...
    def load_data(self, motion_file, min_length=-1):
        ...
        assert (
            len(self._motion_data_load) > 0
        ), f"Failed to load motion data from {motion_file}"
